...
const AICenter: React.FC<{ user: any }> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    checkStatus();
    // مراقبة تغيير الحالة كل فترة قصيرة لضمان التزامن
  ...
  const handleOpenKeySelector = async () => {
    try {
      ...
      console.error("فشل فتح اختيار المفتاح", e);
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
           <button 
            ...
            className={`px-8 py-4 rounded-[1.5rem] flex items-center gap-3 text-sm font-black transition-all shadow-xl active:scale-95 border-2 ${apiKeyStatus === 'connected' ? 'bg-white border-emerald-100 text-slate-900 hover:border-emerald-300' : 'bg-slate-900 text-white border-transparent hover:bg-rose-600'}`}
          >
            {apiKeyStatus === 'connected' ? <ShieldCheck className="w-5 h-5 text-emerald-500" /> : <LinkIcon className="w-5 h-5 text-rose-500" />}
            {apiKeyStatus === 'connected' ? 'تغيير مفتاح API المربوط' : 'ربط مفتاح API الآن'}
          </button>
        </div>
      </div>
